...
class MetaICLModel(object):
    ...
    def load(self):
        ...
        # 支持 Qwen3 等需要 trust_remote_code 的模型
        try:
            from transformers import AutoConfig
            AutoConfig.from_pretrained(model_name, cache_dir=cache_dir, trust_remote_code=True)
            use_trust = True
        except Exception:
            use_trust = False
        ...
        # 检查是否使用 4-bit 量化 (适合大模型如 Qwen3-30B-A3B)
        use_4bit = getattr(self.args, 'load_in_4bit', False)
        if use_4bit:
            ...
            load_kwargs["dtype"] = torch.bfloat16 if torch.cuda.is_available() and torch.cuda.is_bf16_supported() else torch.float16
    ...
    def do_inference(self, data, batch_size=1, verbose=False):
        ...
        losses = []
        for batch in dataloader:
            # 兼容 device_map="auto" 和原始 cuda 方式
            ...
            with torch.no_grad():
                loss = self.run_model(input_ids, attention_mask, token_type_ids, labels=labels)
            losses += loss.cpu().detach().float().numpy().tolist()
        return losses
