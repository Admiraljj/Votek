...
from transformers import GPTJForCausalLM, GPT2Model, GPT2Tokenizer
from collections import OrderedDict
import sqlparse

def calculate_sentence_transformer_embedding(text_to_encode,args):
    try:
        # 尝试使用 sentence transformer
        ...
        print("Using GPT2-xl embeddings as fallback...")
        return calculate_gpt2_embedding(text_to_encode, args)

def calculate_gpt2_embedding(text_to_encode, args):
    """使用 GPT2-xl 计算嵌入的备用方案"""
    from transformers import GPT2Model, GPT2Tokenizer
    import torch

    print("Loading GPT2-xl for embeddings...")
    model = GPT2Model.from_pretrained(args.model_name, cache_dir=args.model_cache_dir)
    tokenizer = GPT2Tokenizer.from_pretrained(args.model_name, cache_dir=args.model_cache_dir)
    ...
    embeddings = []
    num = len(text_to_encode)
    batch_size = 10  # 每次处理10个文本

    with torch.no_grad():
        for i in tqdm(range(0, num, batch_size), desc='calculate GPT2 embeddings'):
            batch_texts = text_to_encode[i:i+batch_size]
